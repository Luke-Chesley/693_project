...
int main(int argc, char *argv[]) {
    ...
    const char* train_data_pattern = "dev/data/fineweb/fineweb10B/fineweb_train_*.bin";
    const char* val_data_pattern = "dev/data/fineweb/fineweb10B/fineweb_val_*.bin";
    //const char* load_filename = "gpt2_124M_bf16.bin"; // bf16 weights of the model
    const char* load_filename = "gpt2:d12"; // train from scratch
    ...
    int print_log_every = 10; // how often to print loss
    ...
    printf0("| inital loss estimate  | %-50f |\n", logf(model.config.vocab_size))
    printf0("+-----------------------+----------------------------------------------------+\n");
    ...
    // tracking total tokens processed
    size_t cumulative_tokens_processed = 0;
    ...
    for (; step <= train_num_batches; step++) {
        ...
        cumulative_tokens_processed += tokens_processed;
        ...
        float cumulative_tokens_in_billions = (float)cumulative_tokens_processed / 1e9f;
        
        // only print out every n steps here
        if (step % print_log_every == 0){
        printf0("step %4d/%d | loss %7.6f (%+.2f)| norm %6.4f (%+.2f)| lr %.2e | %.2f ms | %.1f%% bf16 MFU | %.0f tok/s | %.9f Billion toks\n",
                step + 1, train_num_batches, model.mean_loss, zloss, grad_norm, zgrad, step_learning_rate,
                time_elapsed_ms, 100 * mfu, bias_corrected_ema_tokens_per_second, cumulative_tokens_in_billions);
        }
